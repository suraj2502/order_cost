...
@order_router.post("/get_items_cost2")
async def get_order_items_cost(request:Request,order_details:order_schema):

    total_cost=0 
    print(order_details.order_items)
    print(order_details.distance)
    print(order_details.offer_details)
    ...
    """    
    if order_details["offer_details"]==None:
        return ResponseModel({'total_order_cost':total_cost},200,"cost calculated without any offer" )
    """
    if order_details["offer_details"]!=None:
        if order_details["offer_details"]["offer_type"] == "FLAT":
            total_cost += delivery_cost(order_details["distance"])
            total_cost = max(total_cost - order_details["offer_details"]["offer_value"],0)

        elif order_details["offer_details"]["offer_type"] == "DELIVERY":
            total_cost += delivery_cost(order_details["distance"])
            total_cost = max(total_cost - order_details["offer_details"]["offer_value"],0)
    else:
        total_cost += delivery_cost(order_details["distance"])
